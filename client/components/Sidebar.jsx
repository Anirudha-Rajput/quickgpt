...
const Sidebar = ({isMenuOpen,setIsMenuOpen}) => {
  const { user, theme, setTheme, chats, setSelectedChat, navigate } =useContext(AppContextData);
    ...
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && "max-md:-translate-x-full"} />}`}>
      ...
      <div    className="flex-1 overflow-y scroll mt-3 text-sm space-y-3">
        {chats
          ...
          .map((chat) => (
            <div onClick={()=>{ setSelectedChat(chat);navigate("/"); setIsMenuOpen(false) }}
              key={chat._id}
            ...
            >
              <div >
                ...
                <p className="text-xs text-gray-500 dsark:text-[#B1A6C0] ">
      ...
      {/* user profile*/}
        ...
        className="flex items-center gap-3 mt-4 p-3 border border-gray-300  dark:border-white/15 rounded-md cursor-pointer group"
      >
